...
class CycleGAN():
    def __init__(self,
                 ...
                 # net parameter - default
                 generate_net='9BlockResnet', discriminate_net='PatchGAN',
                 seg_net='9BlockResnet', lr_D=2e-4, lr_G=2e-4, image_shape=(256, 256, 1),
                 # traning/generating parameter - **customed**
                 TrainORGenerate='Train',overlap=[], img_size=[],# required
                 training_mode='2Step', SM=1, # required when training
                 epochnum=0,model_dir='',generate_dir=''): # required when generating

        # ======================Parameters============================
        #Discriminator_learning rate=2e-4, Generator_learning rate=2e-4
        self.training_mode=training_mode
        self.generate_net=generate_net
        self.seg_net=seg_net
        self.discriminate_net=discriminate_net
        self.TrainORGenerate=TrainORGenerate
        self.SM=SM
        self.overlap=overlap  #[A_overlap_w,A_overlap_h,B_overlap_w,B_overlap_h]
        self.img_size=img_size  #[A_w, A_h, B_w, B_h]
        ...
        # Multi scale discriminator - if True the generator have an extra encoding/decoding step to
        # match discriminator information access
        self.use_multiscale_discriminator = False

        # Resize convolution - instead of transpose convolution in deconvolution layers (uk)
        # can reduce checkerboard artifacts but the blurring might affect the cycle-consistency
        ...
        image_A = Input(shape=self.img_shape) #specify input image_A (256,256,1)
        image_B = Input(shape=self.img_shape) #specify input image_B (256,256,1)
        guess_A = D_A(image_A) #specify output guess_A (32,32,1)
        guess_B = D_B(image_B) #specify output guess_B
        ...
        # Generators
        if self.generate_net == '9BlockResnet':
            self.G_A2B = self.modelGenerator(name='G_A2B_model')
            self.G_B2A = self.modelGenerator(name='G_B2A_model')
        elif self.generate_net=='UNet': # get_unet函数还有点儿问题
            self.G_A2B = self.get_unet(16, 0.4, True, name='G_A2B')
            self.G_B2A = self.get_unet(16, 0.4, True, name='G_B2A')
        ...
        if self.training_mode == 'E2E':
            if self.seg_net == '9BlockResnet':
                self.seg = self.modelGenerator(name='seg')
            elif self.seg_net == 'UNet':
                self.seg = self.get_unet(name='seg')

            # segmentor builds
            segment_B = self.seg(synthetic_B)


        # ======== paras append ========
        if self.training_mode=='2Step':
        ...
        elif self.training_mode=='E2E':
            model_outputs = [reconstructed_A, reconstructed_B, segment_B]
            compile_losses = [self.cycle_loss, self.cycle_loss, self.dice_coef_loss, self.lse, self.lse]
            compile_weights = [self.lambda_1, self.lambda_2, 1.0, self.lambda_D, self.lambda_D]
        ...
        if self.use_data_generator:
            ...
            print('--- Caching data ---') #数据缓存
        sys.stdout.flush() #缓存区刷新
        ...
        elif self.TrainORGenerate=='Generate':
            self.load_model_and_generate_synthetic_images(epochnum=self.epochnum)
    ...
    def train(self, epochs, batch_size=1, save_interval=1):
        def run_training_iteration(loop_index, epoch_iterations):
            ...
            if self.training_mode=='2Step':
                ...
                target_data = [real_images_A, real_images_B, real_images_A]
        ...
        # Begin training
        # ======================================================================
        #OrderedDict(): Sort elements in dictionary objects
        ...
        if self.use_multiscale_discriminator:
            ...
            label_shape = (batch_size,) + self.D_A.output_shape[1:] #tuple (1,32,32,1)
        ...
        start_time = time.time()   # Return the current time in seconds since the Epoch.
    ...
    def ck(self, x, k, use_normalization, stride):
        "x: input; k: filter number; use_normalization: true/false; stride: convolution layer stride"
        "conv+normalization+leakyReLU"
        x = Conv2D(filters=k, kernel_size=4, strides=stride, padding='same')(x)
        # Normalization is not done on the first discriminator layer
        if use_normalization:
            x = self.normalization(axis=3, center=True, epsilon=1e-5)(x, training=True)
            "epsilon:ε Small float added to variance to avoid dividing by zero."
        x = LeakyReLU(alpha=0.2)(x)
        "f(x) = alpha * x for x < 0`, f(x) = x for x >= 0."
        return x

    def c7Ak(self, x, k):
        "x: input; k: filter number"
    ...
    def modelDiscriminator(self, name=None):
        # Specify input
        input_img = Input(shape=self.img_shape) #used to instantiate a Keras tensor
        # Layer 1 (#Instance normalization is not used for this layer)
        # filter number=64, kernel size=4, stride=2
        x = self.ck(input_img, 64, False, 2)
        # Layer 2
        # filter number=128, kernel size=4, stride=2
        x = self.ck(x, 128, True, 2)
        # Layer 3
        # filter number=256, kernel size=4, stride=2
        x = self.ck(x, 256, True, 2)
        # Layer 4
        # filter number=512, kernel size=4, stride=1
        x = self.ck(x, 512, True, 1)
        # Output layer
        if self.use_patchgan:  # PatchGAN - if false the discriminator learning rate should be decreased
            x = Conv2D(filters=1, kernel_size=4, strides=1, padding='same')(x)
        else:
            x = Flatten()(x) #Flattens the input.turn multi-dimension into one dimension
            x = Dense(1)(x) #dense to 1
    ...
    def lse(self, y_true, y_pred):
        loss = tf.reduce_mean(tf.squared_difference(y_pred, y_true)) #returns (x - y)(x - y) element-wise.
        return loss
    ...
    def truncateAndSave(self, real_, real, synthetic, reconstructed, path_name):
        ...
        Image.fromarray((image*255).astype('uint8'),"L").save(path_name)
    ...
    def load_model_and_weights(self, model, epochnum):
        # path_to_model = os.path.join('generate_images', 'models', '{}_weights_epoch_40.json'.format(model.name))

        path_to_weights = os.path.join('saved_models',self.model_dir,
                                       '{}_weights_epoch_{}.hdf5'.format(model.name,epochnum))
        #                                                                                               *remember to change path
        # model = model_from_json(path_to_model)
        model.load_weights(path_to_weights)

    def load_model_and_generate_synthetic_images(self,epochnum):
        def save_image(image, name, domain):
            if self.channels == 1:
                image = image[:, :, 0]
            Image.fromarray((image*255).astype('uint8'),"L").\
                save(os.path.join('generate_images',self.generate_dir,domain, name))

        directory = os.path.join('generate_images',self.generate_dir)
        #                                       *remember to change path
        if not os.path.exists(os.path.join(directory, 'A_train')):
            os.makedirs(os.path.join(directory, 'A_synthetic'))
            os.makedirs(os.path.join(directory, 'B_synthetic'))
            os.makedirs(os.path.join(directory, 'A_train'))


        self.load_model_and_weights(self.G_A2B,epochnum)
        self.load_model_and_weights(self.G_B2A,epochnum)
        synthetic_images_B = self.G_A2B.predict(self.A_train)
        synthetic_images_A = self.G_B2A.predict(self.B_train)
        full_pred = reconstract_image_from_patch_overlap(synthetic_images_B, self.img_size[0], self.img_size[1],
                                                              256, 256,
        ...
        for i in range(len(synthetic_images_A)):
            ...
            name = str(i) + '_synthetic.png'
            synt_A = synthetic_images_A[i]
            save_image(synt_A, name, 'A_synthetic')

        for i in range(len(full_pred)):
            # Get the name from the image it was conditioned on
            name = str(i) + '_fullpred.png'
            full_pred_B = full_pred[i]
            save_image(full_pred_B, name, 'B_synthetic')
        ...
        for i in range(len(synthetic_images_B)):
            ...
            save_image(synt_B, name, 'B_synthetic')
            name1=str(i)+'_train.png'
            input_B=self.A_train[i]
            save_image(input_B,name1,'A_train')

        print('{} synthetic images have been generated and placed in %s'%directory
...
if __name__ == '__main__':
    ...
    '''
    *** Customized training/generating parameters:
        0-required in both mode:
            - TrainORGenerate: Training mode or Generating mode
            - img_size: [A_width, A_height, B_width, B_height]
            - overlap: [A_overlap_width, A_overlap_height, B_overlap_width, B_overlap_height]
        1-required in Training mode:
            - training_mode: '2Step' (default) or 'E2E'
            - SM: Size matching method
                  0-no size matching, default
                  1-vessel dialation
                  2-threshold selection
        2-required in Generating mode:
            - epochnum: epoch number of selected model
            - model_dir: folder where selected model in 
            - generate_dir: folder where generated images in
    '''
    GAN = CycleGAN(TrainORGenerate='Generate',img_size=[565,584,280,400],overlap=[62,82,24,72],
                   training_mode='2Step',SM=0,
                   epochnum=100,model_dir='model_test_dir',generate_dir='generate_images_dir')